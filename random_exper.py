...
def main():
    ...
    parser.add_argument('--random_exper', action='store_true', default=True,
                        help="indicate random experiment")
    ...
    random_ckpt_dir = cfg.ckpt_dir + '/random' + str(args.forget_num)

    if not os.path.exists(random_ckpt_dir):
        os.makedirs(random_ckpt_dir)

    set_seed(cfg.SEED)

    log_file = random_ckpt_dir + '/log'
    if args.forget_type is not None and args.fine_prune:
        log_file = "{}/forget_{}_{}_fineprune_log".format(random_ckpt_dir, args.forget_type,
                                                             args.forget_num)
    elif args.forget_type is not None and not args.fine_prune:
        log_file = "{}/forget_{}_{}_log".format(random_ckpt_dir, args.forget_type, args.forget_num)
    ...
    trainset = get_dataset(dataset_name, root=os.path.join(cfg.data_dir, cfg.DATASET),
                           train=True, download=True, data_augment=args.data_augment,
                            forget_type=args.forget_type, forget_num=args.forget_num, random_exper=True)
    ...
    LAST_MODEL = "{}/last.pt".format(random_ckpt_dir)
    BEST_MODEL = "{}/best.pt".format(random_ckpt_dir)

    if not args.no_train:
        last_model = LAST_MODEL if args.forget_type is None else \
            "{}/forget_{}_{}_last.pt".format(random_ckpt_dir, args.forget_type, args.forget_num)
        best_model = BEST_MODEL if args.forget_type is None else \
            "{}/forget_{}_{}_best.pt".format(random_ckpt_dir, args.forget_type, args.forget_num)
    ...
    if args.fine_prune:
        ...
        model0_file = "{}/forget_{}_{}_best.pt".format(random_ckpt_dir, args.forget_type,
                                                       args.forget_num)
        ...
        forget_dataset = get_dataset(dataset_name, root=os.path.join(cfg.data_dir, cfg.DATASET),
                                     ...
                                     only_forget=True, random_exper=True)
        ...
        full_train_dataset = get_dataset(dataset_name, root=os.path.join(cfg.data_dir, cfg.DATASET),
                                     train=True, download=True, data_augment=args.data_augment)
        correct_forget_dataset = torch.utils.data.Subset(full_train_dataset, forget_dataset.random_id)
        correct_forget_dataloader = DataLoader(correct_forget_dataset, batch_size=cfg.N_BATCH, shuffle=True,
                              num_workers=cfg.N_WORKER, worker_init_fn=worker_init_fn)

        remain_dataset, remain_loader = trainset, train_loader

        test_dataset = get_dataset(dataset_name, root=os.path.join(cfg.data_dir, cfg.DATASET), train=False,
                                            download=True, data_augment=args.data_augment,)
        test_loader = DataLoader(test_dataset, batch_size=cfg.N_BATCH, shuffle=False,
                                        num_workers=cfg.N_WORKER, worker_init_fn=worker_init_fn)
        ...
        '''ti_finetune_model_path = "{}/finetune_baseline_pruned_{}_{}_model.pth". \
            format(random_ckpt_dir, args.forget_type, args.forget_num)
        if not os.path.isfile(ti_finetune_model_path):
            ...
            ti_test_acc_begin = test(None, ti_pruned_net, test_loader, loss_fn)
            ti_forget_acc_begin = test(None, ti_pruned_net, correct_forget_dataloader, loss_fn)

            model_ti, ti_acc_history, ti_train_loss, ti_forget_acc = \
                fine_tune(logger, cfg, args, ti_pruned_net, None, remain_loader,
                          test_loader, correct_forget_dataloader, loss_fn, test_acc,
        ...
        if not os.path.exists(random_finetune_model_path):
            ...
            random_test_acc_begin = test(None, random_model_mask, test_loader, loss_fn)
            random_forget_acc_begin = test(None, random_model_mask, correct_forget_dataloader, loss_fn)
            t1 = time.time()
            random_model_mask, random_acc_history, random_train_loss, random_forget_acc = \
                fine_tune(logger, cfg, args, random_model_mask, None, remain_loader,
                               test_loader, correct_forget_dataloader, loss_fn,
        ...
        print(max(random_acc_history[:5]), max(random_forget_acc[:5]))
        ...
        if not os.path.exists(act_mask_finetune_model_path):
            ...
            test_acc_begin = test(None, act_model_mask, test_loader, loss_fn)
            forget_acc_begin = test(None, act_model_mask, correct_forget_dataloader, loss_fn)
            t3 = time.time()
            act_model_mask, act_acc_history, act_train_loss, act_forget_acc = \
                fine_tune(logger, cfg, args, act_model_mask, None, remain_loader,
                               test_loader, correct_forget_dataloader,
        ...
        print(max(act_acc_history[:5]), max(act_forget_acc[:5]))
        ...
        if not os.path.exists(fisher_mask_finetune_model_path):
            ...
            test_acc_begin = test(None, fisher_model_mask, test_loader, loss_fn)
            forget_acc_begin = test(None, fisher_model_mask, correct_forget_dataloader, loss_fn)
            t3 = time.time()
        ...
        print(max(fisher_acc_history[:5]), max(fisher_forget_acc[:5]))
        ...
        if not os.path.exists(grad_mask_finetune_model_path):
            ...
            test_acc_begin = test(None, grad_model_mask, test_loader, loss_fn)
            forget_acc_begin = test(None, grad_model_mask, correct_forget_dataloader, loss_fn)
            t3 = time.time()
            grad_model_mask, grad_acc_history, grad_train_loss, grad_forget_acc = \
                fine_tune(logger, cfg, args, grad_model_mask, None, remain_loader,
                          test_loader, correct_forget_dataloader,
                          loss_fn, test_acc, epochs=args.finetune_epochs)
            t4 = time.time()
            logger.info("finetune {} epochs, time :{}".format(args.finetune_epochs, t2 - t1 + t4 - t3))
        ...
        print(max(grad_acc_history[:5]), max(grad_forget_acc[:5]))
        logger.info("grad mask ok")

        '''modelf_file = "{}/fisher_baseline_{}_{}_model.pt".format(random_ckpt_dir, args.forget_type,
                                                               args.forget_num)
        ...
        if not os.path.isfile(modelf_file):
            ...
            model_hessian_file = "{}/model_hessian_remain_{}_{}.pt".format(random_ckpt_dir,
                                                                           args.forget_type,
                                                                           args.forget_num)
            ...
            test_acc_f = test(None, modelf, test_loader, loss_fn)
            forget_acc_f = test(None, modelf, correct_forget_dataloader, loss_fn)
            torch.save({
        ...
        finetune_file = "{}/finetune_baseline_{}_{}.pt".format(random_ckpt_dir, args.forget_type,
                                                               args.forget_num)

        if not os.path.isfile(finetune_file):
            model_ft = copy.deepcopy(model)
            model_ft.set_optimizer(cfg, args)
            ft_test_acc_begin = test(None, model_ft, test_loader, loss_fn)
            ft_forget_acc_begin = test(None, model_ft, correct_forget_dataloader, loss_fn)
            t1 = time.time()
        ...
        print(max(ft_acc_history[:5]), max(ft_forget_acc[:5]))
        ...
        np.save("{}/dict_{}_{}.npy".format(random_ckpt_dir, args.forget_type, args.forget_num), pic_data)
